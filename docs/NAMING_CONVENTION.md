...
远程：`git@github.com:<redacted>/PercepFlex.git`（SSH）。
...
回滚兜底：`tar xzf /tmp/naming_bak/v3_<HHMM>/tree.tgz -C ~/ai_study/trac`
...
## 12.6 判据盲区：模块名引用（非路径引用）—— eval 链静默中断

**发生时间**：2026-09-10 16:52（提交 `b0f8d4c`）引入 → 19:49 首次暴露 → 21:2x 定位并修复。

### 12.6.1 事实链

提交 `b0f8d4c`（naming v3）执行了 93 处 `git mv`，其中 **10 个 `.py` 文件改了文件名**：

| 旧路径 | 新路径 |
|---|---|
| `scripts/load_baseline_weights.py` | `scripts/phase2_load_baseline_weights.py` |
| `scripts/precompute_teacher.py` | `scripts/phase2_precompute_teacher.py` |
| `scripts/gen_figures.py` | `scripts/phase2_gen_figures.py` |
| `scripts/smoke_baselines.py` | `scripts/phase2_smoke_baselines.py` |
| `scripts/verify_dataset.py` | `scripts/phase2_verify_dataset.py` |
| `scripts/exp5_pareto.py` | `scripts/phase1_exp5_pareto.py` |
| `scripts/expD_analyze.py` | `scripts/phase2b_expd_analyze.py` |
| `scripts/expFG_analyze.py` | `scripts/phase2b_expfg_analyze.py` |
| `scripts/expA_analyze.py` | `scripts/phase2c_expa_analyze.py` |
| `scripts/bench_latency.py` | `scripts/phase4c_bench_latency.py` |

其中 **仅 `load_baseline_weights` 留下了悬空引用**（其余 9 个无代码级引用）。共 3 处代码引用，
**全部未同步**：

| 文件:行 | 引用 | 影响 |
|---|---|---|
| `evaluation/evaluate_baseline.py:44` | `from load_baseline_weights import ...` | **所有 eval 主线** |
| `training/train.py:317` | `from load_baseline_weights import load_baseline` | 跨架构 KD 教师加载（本阶段 config 未启用，休眠） |
| `scripts/phase2_precompute_teacher.py:33` | `from load_baseline_weights import load_baseline` | 教师特征预计算 |

### 12.6.2 时间线证据

| 时刻 | 事件 | 证据 |
|---|---|---|
| 15:45 | **最后一次成功 eval** | `experiments/phase6/exp6_e8_unif20_eval/metrics.json`（641 B） |
| 16:52 | naming v3 重命名落地 | `b0f8d4c` |
| 19:49 | **第一次失败 eval** | `exp9_aunif_km_eval/eval_stdout.log` → `ModuleNotFoundError: No module named load_baseline_weights` |
| 19:49 | 假行写入结果表 | `phase6c_e9_factorial.csv` 出现 `aunif_km,...,failed-no-metrics` |
| 21:2x | 定位 + 修复 + 补评 | 本节 |

### 12.6.3 为什么门禁没拦住（判据盲区，不是执行疏漏）

`refs_pass2.py` 与 `check_naming.sh` CHK11 的匹配对象是**路径引用**：形如 `a/b/c.py`、带扩展名、
或至少带目录分隔符。而：

- **Python 模块名不是路径**：`from load_baseline_weights import ...` 中没有分隔符、没有扩展名；
- CHK11 的定义要求指向一个已被重命名**且仍在磁盘上可解析**的路径 —— 模块名两个条件都不满足；
- 因此这类引用**既不报 FAIL 也不报 WARN**，静默通过。失效模式与本章开头的 `.gitignore` 事件同源：
  **重命名 → 引用静默失效 → 默认动作（这里是 eval）在无人注意时改变行为**。
- 补记：`b0f8d4c` 提交信息里已记录 "19 unresolvable"，但那是**历史引用清单**
  （`scripts/naming_historical_refs.txt`，18/19 是从未入库的文献引用）。本次的 3 处**不在这 19 条里** ——
  即它们从未被任何清单登记，是纯盲区。

**已识别的待补门禁（建议，待批准，不静默实施）**：新增 `CHK12`（或 `A5`）——
项目自带代码中出现的**裸模块名 import** 必须在项目 `sys.path` 上可解析，外部依赖走白名单；
反例自检为植入 `from old_module_name import x` 必须变红。

### 12.6.4 处置

- 4 处引用改为 `phase2_load_baseline_weights`（含 `scripts/phase2_load_baseline_weights.py:11` 的 docstring 示例）；
- 修复后 `python evaluation/evaluate_baseline.py --help` 退出码 0（`datasets` / `profiling` /
  `evaluation` / 权重加载器 整条 import 链实测通过）；
- 新增 `scripts/phase6c_e9_recover_aunif.sh`：等训练链退出后**只补 eval**（复用 checkpoint，**不重训**），
  用 train 侧字段（wall / peak_mem / final_loss）+ 真实 metrics 替换 `failed-no-metrics` 假行；
  幂等（已补评则 no-op）；日志写 `experiments/phase6c/recover_aunif.log`。

### 12.6.5 记录限制（如实留痕）

- 本次补评 eval 在**工作树含未提交修复**的状态下执行。该修复是**纯模块名改名**，
  不触碰任何数值路径（`train.py:317` 在本阶段 config 未启用），因此结果表 `git_commit` 保持
  **9fb29e7**（cell 1 训练时的 HEAD），**不制造 SHA 分叉**，避免同一析因的两格被误读成"跨代码版本"。
- 失败现场保留：`experiments/phase6c/exp9_aunif_km_eval.log` 是唯一未被补评覆盖的
  `ModuleNotFoundError` 现场，清理时**特意排除**，不删。
