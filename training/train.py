...
def gpu_usage(idx=0):
    ...
    try:
        ...
        pass
    try:  # device-wide: informational, and the historical format other scripts parse
        import subprocess
        out = subprocess.run(
        ...
        if out.returncode == 0:
            ...
            if not any(p.startswith("gpu%") for p in parts):
                parts.append(f"gpu% {u:>3}")
            parts.append(f"mem {mu}/{mt}MiB")
    except Exception:
        pass
    try:  # per-process peaks -- AUTHORITATIVE for the VRAM-cliff guard
        a = torch.cuda.max_memory_allocated(idx) / 1024**2
        r = torch.cuda.max_memory_reserved(idx) / 1024**2
        parts.append(f"self {a:.0f}/{r:.0f}MiB")
    except Exception:
        pass
    if not parts:
        parts.append("gpu?")
...
def main():
    ...
    if kd_cfg and not kd_offline:
        ...
        if tname == "YOLOP":
            ...
            from phase2_load_baseline_weights import load_baseline
    ...
    if args.resume:
        ...
        if os.path.exists(rp):
            ...
            rng = rck.get("rng")
            if rng:
                # torch.set_rng_state / torch.cuda.set_rng_state_all / Generator.set_state
                # all REQUIRE a CPU uint8 tensor ("ByteTensor").  The torch.load above used
                # map_location=device, which moved EVERY tensor in the file -- these RNG
                # byte-vectors included -- onto the GPU, so each of the calls below raised
                #     TypeError: RNG state must be a torch.ByteTensor
                # 2026-09-14: this killed the seed-1 resume 17 s into the run.  The round-2
                # CPU self-test could not have caught it: on device=cpu map_location is a
                # no-op, so the defect is CUDA-only.  The coercion is deliberately LOCAL to
                # the RNG payload -- loading the whole checkpoint with map_location="cpu"
                # would also drag the Adam moment buffers to the CPU and change what the
                # optimizer sees.
                def _rng_cpu(t):
            ...
            # FAIL CLOSED: a checkpoint that loads but carries no epoch would silently
            # continue as a "fresh" run from epoch 1, and its result would then be filed
            # under the wrong label.  Refuse instead.  (Same failure class as the withdrawn
            # relative-path bug, which produced exactly such an unintended fresh run.)
            if start_epoch <= 1:
                print(f"[train] FATAL: --resume {rp} loaded but start_epoch={start_epoch}; "
                      f"refusing to silently restart from scratch", flush=True)
                sys.exit(2)
        else:
