...
if [ -f "$MP" ]; then
  echo "[r4] ${TAG} metrics present - eval reuse" | tee -a "$OUT/final_chain.log"
else
  mkdir -p "$TR" "$EVL"
  T0=$(date +%s)
  RESUME=""
  if [ -f "$CKPT" ]; then RESUME="--resume $CKPT"; fi
  echo "===== [$(date '+%F %T')] ${TAG} TRAIN start (ep=${EP} seed=${SEED} ${RESUME:-fresh}) =====" | tee -a "$OUT/final_chain.log"
  # shellcheck disable=SC2086
  "$PY" training/train.py --config "$CFG" --outdir "$TR" --epochs "$EP" --seed "$SEED" $RESUME \
    2>&1 | tee "$TLOG" > "$TR/train_stdout.log"
  T1=$(date +%s)
  WALL_MIN=$(( (T1 - T0) / 60 ))
  echo "===== [$(date '+%F %T')] ${TAG} TRAIN done in ${WALL_MIN} min =====" | tee -a "$OUT/final_chain.log"
  ...
  if [ "$SELFN" -ge 1 ]; then
    ...
    echo "[r4] ${TAG} peak SELF ${PEAK_USED}MiB / ${PEAK_DEN}MiB = ${PCT}% (samples=${SELFN})" | tee -a "$OUT/final_chain.log"
  else
    ...
    echo "[r4] ${TAG} peak DEVICE-WIDE ${PEAK_USED}MiB / ${PEAK_DEN}MiB = ${PCT}% (samples=${MEMN}; FALLBACK - no per-process field in this log)" | tee -a "$OUT/final_chain.log"
    if [ "${MEMN:-0}" -lt 1 ]; then
      echo "[r4] ABORT: ${TAG} produced no parseable mem samples - VRAM guard UNVERIFIED (fail-closed)" | tee -a "$OUT/final_chain.log"
      echo "${TAG}_ABORTED_unverified_vram" >> "$OUT/ABORTS.txt"
      exit 2
    fi
  fi
  if [ "${PCT:-0}" -gt 96 ]; then
    echo "[r4] ABORT: ${TAG} exceeded the 96% VRAM cliff threshold (${PCT}%) - result would be silently paged, not valid" | tee -a "$OUT/final_chain.log"
...
if [ ! -f "$MP" ]; then
  ...
  echo "[FAIL] ${TAG} no metrics.json" | tee -a "$OUT/final_chain.log"
  exit 1
fi
...
PYEOF
echo "[r4] ${TAG} DONE" | tee -a "$OUT/final_chain.log"
