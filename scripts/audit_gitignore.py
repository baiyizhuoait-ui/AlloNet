...
# Root-anchored literal rules exempt from the DEAD-RULE report. These name RUNTIME
# CONTROL FILES: the chains create and remove them as they run, so "not on disk at
# this instant" is their normal state. Reporting them as rename leftovers would push
# someone to delete the rule, and then the next `touch STOP_CHAIN` (our polite abort
# lever -- scripts/phase6_final100_run.sh:37) could be committed and halt the next run.
# The DEAD-RULE check targets rename leftovers only; runtime markers belong here.
RUNTIME_EXEMPT = frozenset({
