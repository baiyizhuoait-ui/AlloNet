#!/bin/bash
# Phase 6 overnight P6A-STEP2 chain (2026-09-10 01:30 -> ~08:00)
# EXP-04: combo (danc anchors + l14f1 lane 1/4) 4ep screen -> 20ep confirm (gated)
# EXP-01: lch72 (FLOPs-parity channel control) 4ep -> 20ep only if it beats l14f1 4ep
...
echo "[chain] GATE-6A.1 combo4: det=$COMBO_DET lane=$COMBO_LANE -> combo20ep=$RUN_COMBO20"

if [ "$RUN_COMBO20" = "1" ]; then
  echo "[chain] P6A-STEP2c: combo 20ep confirmation"
  maybe_stop
...
echo "[chain] GATE-6A.2 lch72 4ep lane_fg=$LCH_FG (l14f1 ref 0.1916) -> lch20ep=$RUN_LCH20"

if [ "$RUN_LCH20" = "1" ]; then
  echo "[chain] P6A-STEP2d: lch72 20ep (channel arm wins 4ep - needs confirm)"
  maybe_stop
