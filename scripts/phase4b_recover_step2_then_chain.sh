...
log "RECOVERY START: continue P4B5-STEP2 (danc seed2) from ep19 ckpt, then full chain P4B5-STEP3-5"
if awk -F, 'NR>1 && $2=="danc_z16" && $18==2 {f=1} END{exit !f}' "$CSV" 2>/dev/null; then
  log "P4B5-STEP2 seed2 row already present - skip recovery"
else
  log "P4B5-STEP2: train +1 epoch from ep19 backup (genuine continuation -> 20ep)"
  ...
  log "P4B5-STEP2: patch checkpoint epoch metadata 1 -> 20"
  ...
  log "P4B5-STEP2: runner eval-only (CK_EP==20) -> appends det_e20 row"
  ...
  log "P4B5-STEP2 recovery done"
fi
log "Now run full chain (P4B5-STEP1/2 skipped via resume guards; P4B5-STEP3 dp2b / P4B5-STEP4 lane8 / P4B5-STEP5 da14)"
