...
from phase2_load_baseline_weights import load_twinlitenet, load_twinlitenetplus, load_trilitenet  # noqa: E402
...
def build_yolop(device):
    ...
    # Round 3 / EXP-9A (2026-09-10): optional anchor override.  The arms of the
    # second-architecture anchor experiment differ ONLY by their 9 anchors, so the
    # set must travel with the checkpoint.  Without this, eval would silently
    # decode with the shipped set and every arm would score identically -- the
    # exact silent-substitution failure class this repo has already been bitten by
    # twice.  The path is passed by env var because this CLI has no anchor flag;
    # an unset/invalid path is fatal rather than ignored, so a typo cannot quietly
    # fall back to the shipped anchors.
    # ------------------------------------------------------------------
    # Round 3 / EXP-9A trained-arm weights override (added 2026-09-11).
    #
    # WHY THIS EXISTS: the first STEP-2 run evaluated all three anchor arms
    # against the *released* weights.  build_yolop had no way to receive a
    # checkpoint, so the "arms" differed only in the decode anchor set and every
    # arm's lane/DA metric came out bit-identical -- the signature of a silent
    # substitution.  The three EXP-9A rows produced that way were discarded.
    #
    # The two env vars are the arm's identity: YOLOP_WEIGHTS carries the trained
    # model, YOLOP_ANCHORS_JSON carries the checkpoint's own anchor set.  Passing
    # anchors without weights is now a hard error (it *is* the bug), unless
    # YOLOP_ALLOW_RELEASED=1 explicitly asks for the released-model reference.
    # ------------------------------------------------------------------
    _wp = os.environ.get("YOLOP_WEIGHTS")
    _ap = os.environ.get("YOLOP_ANCHORS_JSON")
    if _ap and not _wp and os.environ.get("YOLOP_ALLOW_RELEASED") != "1":
        raise RuntimeError(
            "YOLOP_ANCHORS_JSON set without YOLOP_WEIGHTS: that combination decodes "
            "the released model with a foreign anchor set and silently produces a "
            "meaningless metric (the 2026-09-11 EXP-9A failure).  Pass the arm's "
            "checkpoint via YOLOP_WEIGHTS, or set YOLOP_ALLOW_RELEASED=1 if you "
            "really mean the released-model reference.")
    if _wp:
        if not os.path.exists(_wp):
            raise FileNotFoundError(f"YOLOP_WEIGHTS={_wp} does not exist")
        _ck = torch.load(_wp, map_location="cpu", weights_only=False)
        _sd = _ck.get("model_state", _ck)
        _missing, _unexpected = model.load_state_dict(_sd, strict=False)
        if _missing or _unexpected:
            raise RuntimeError(
                f"YOLOP_WEIGHTS mismatch: missing={list(_missing)[:5]} "
                f"unexpected={list(_unexpected)[:5]}")
        # Self-proof: a "trained" checkpoint that is bit-identical to the released
        # weights would produce a fake trained metric.  Compare against the
        # released dict (still in scope as `sd`) and refuse if nothing changed.
        _n_cmp, _dmax = 0, 0.0
        for _k, _v in _sd.items():
            _r = sd.get(_k)
            if _r is None or not hasattr(_r, "shape") or _r.shape != _v.shape:
                continue
            _n_cmp += 1
            _dmax = max(_dmax, (_v.float() - _r.float()).abs().max().item())
        print(f"[yolop] WEIGHTS OVERRIDE {_wp} "
              f"(trained-vs-released max|delta|={_dmax:.4e} over {_n_cmp} tensors)")
        if _n_cmp == 0 or _dmax == 0.0:
            raise RuntimeError(
                "YOLOP_WEIGHTS loaded but is bit-identical to the released weights "
                f"(compared {_n_cmp} tensors) - refusing to report a fake trained metric")
    if _ap:
        if not os.path.exists(_ap):
            raise FileNotFoundError(f"YOLOP_ANCHORS_JSON={_ap} does not exist")
        _px = torch.tensor(json.load(open(_ap)), dtype=torch.float32)
        _det = model.model[model.detector_index]
        _n, _a, _ = _px.shape
        assert (_n, _a) == (_det.nl, _det.na), "anchor override shape mismatch"
        with torch.no_grad():
            _det.anchor_grid.copy_(_px.view(_n, 1, _a, 1, 1, 2))
            _det.anchors.copy_(_px / _det.stride.view(-1, 1, 1))
        print(f"[yolop] ANCHOR OVERRIDE {_ap}: {_px.reshape(-1, 2).tolist()}")
...
_R3 = {"arm": None, "weights": None}


def build_round3_tlp(device):
    """Round 3 / EXP-9B: evaluate a TRAINED lane arm.

    Reuses this file's whole eval protocol on purpose (letterbox 640, pad-crop,
    argmax over 2 channels, SegmentationMetric) so an arm's number is directly
    comparable with the released-checkpoint baseline computed by the same code
    path.  A separate evaluator would risk silent protocol drift.
    """
    sys.path.insert(0, os.path.join(ROOT, "baselines", "TwinLiteNetPlus"))
    from models.round3.tlp_variants import Round3TLP  # noqa: E402
    if not _R3["weights"] or not os.path.exists(_R3["weights"]):
        raise FileNotFoundError(f"--round3-weights missing/invalid: {_R3['weights']}")
    preset = _R3["preset"] or "small"
    model = Round3TLP(preset, _R3["arm"])
    ck = torch.load(_R3["weights"], map_location="cpu", weights_only=False)
    sd = ck.get("model_state", ck)
    missing, unexpected = model.load_state_dict(sd, strict=False)
    if missing or unexpected:
        raise RuntimeError(f"round3 ckpt mismatch: missing={list(missing)[:5]} unexpected={list(unexpected)[:5]}")
    print(f"[round3] arm={_R3['arm']} preset={preset} loaded {_R3['weights']}")
    return model.to(device), "unit"
...
def build(name, preset, device):
    ...
    if name == "Round3TLP":
        return build_round3_tlp(device)
    if name == "TwinLiteNet":
...
def normalize_batch(x, norm):
    """SINGLE SOURCE OF TRUTH for input normalisation.

    Both this evaluator AND the Round-3 trainers must call this, so that train
    and eval can never drift apart -- that drift was exactly the 2026-09-11
    EXP-9A failure: the trainer fed raw [0,1] tensors while the evaluator fed
    ImageNet-normalised ones, so every "trained" arm was evaluated on a tensor
    distribution it had never been optimised for (released mAP 0.7712 -> 0.07).
    ...
    if norm == "unit":
        return x
    raise ValueError(f"unknown norm {norm!r} (expected 'imagenet' or 'unit')")


def preprocess(img, norm):
    x = img.unsqueeze(0).to("cuda" if torch.cuda.is_available() else "cpu")
    return normalize_batch(x, norm)
...
def main():
    ...
    ap.add_argument("--baseline", required=True,
                    choices=["YOLOP", "TwinLiteNet", "TwinLiteNetPlus", "TriLiteNet",
                             "OursStatic", "OursDynamic", "Round3TLP"])
    ap.add_argument("--preset", default=None, help="nano/small/medium/large (TLP) or tiny/small/base (TriLiteNet)")
    ap.add_argument("--round3-arm", default=None, choices=["baseline", "spatial", "channel"],
                    help="Round 3 / EXP-9B lane arm to evaluate (baseline|spatial|channel)")
    ap.add_argument("--round3-weights", default=None, help="checkpoint.pt produced by phase6_round3_tlp_train.py")
    ...
    if args.baseline == "Round3TLP":
        # must be set before build(); a missing arm/weights pair is fatal, never a
        # silent fallback to the released checkpoint (that would score an untrained
        # model and look like a valid arm).
        if not args.round3_arm or not args.round3_weights:
            raise SystemExit("--baseline Round3TLP requires --round3-arm and --round3-weights")
        _R3.update(arm=args.round3_arm, weights=args.round3_weights, preset=args.preset or "small")
    ...
    # ---------------- dataset ----------------
    ds = BDD100KDataset(args.data_root, split=args.split,
                        with_det=args.baseline not in ("TwinLiteNet", "TwinLiteNetPlus", "Round3TLP"),
