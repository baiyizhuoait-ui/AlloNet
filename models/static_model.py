...
from models.encoder.factory import build_encoder
...
class StaticMultiTaskModel(nn.Module):
    def __init__(self, model_cfg):
        ...
        # Dispatch on enc_cfg.get("arch"), defaulting to LightEncoder.
        # A config WITHOUT an `arch` key therefore builds exactly as before --
        # proven bit-for-bit (keys, param count, seeded forward) by
        # scripts/phase6_g3_arch_probe.py --equivalence, not asserted here.
        # Exports must still be [f1(1/4), F2(1/8), F3(1/16), F4(1/32)] with
        # channel widths == enc_cfg["stages"], because the lines below index it.
